...
from src.models.pharos_pretrained_drug import PharosPretrainedDrugModel
...
def train_one_epoch(model, loader, optimizer, loss_fn, device):
    ...
    for batch in loader:
        ...
        pred = model(drug_embedding, cell_expr, resistance_expr)
        loss = loss_fn(pred, label)

        loss.backward()
        optimizer.step()

        total_loss += loss.item() * drug_embedding.size(0)
...
def main():
    ...
    model = PharosPretrainedDrugModel(
        drug_embedding_dim=sample["drug_embedding"].shape[0],
    ...
    pd.DataFrame(history).to_csv(
        output_dir / "pharos_pretrained_drug_training_history.csv",
        index=False,
    )
    pd.DataFrame([best_metrics]).to_csv(
        output_dir / "pharos_pretrained_drug_best_metrics.csv",
        index=False,
    )

    print("\nBest pretrained-drug PHAROS metrics:")
    print(best_metrics)
